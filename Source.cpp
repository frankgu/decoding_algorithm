...
void applySumProduct(std::vector<std::vector<std::vector<double>>> &prob,
	...
	while (_start != 0)
	{
		...
		*/
		for (int i = 0; i < _num; i++)
		{
			...
			std::vector<std::vector<double>> Num2;
			for (int j = 0; j < 7; j++)
			{
				...
				for (int m = 0; m < 2; m++)
				{
					...
					temp2.push_back(Util::instance()->Xprobability(_S, m, _Z[i][j]));
				}
				// normalize the value
				temp2[0] = temp2[0] / (temp2[0] + temp2[1]);
				temp2[1] = temp2[1] / (temp2[0] + temp2[1]);
			...
			for (int i = 0; i < _num; i++)
			{
				...
				for (int j = 0; j < 7; j++)
				{
					if (j == 0){
						...
						applySumProduct(_prob, otherValues, i, j);
						// max product algorithm
						applyMaxProduct(_prob_max, otherValues, i, j);
					}
					else if (j == 1)
					{
					...
					else if (j == 2)
					{
						...
						// sum product algorithm
						applySumProduct(_prob, otherValues2, i, j);
						// max product algorithm
						applyMaxProduct(_prob_max, otherValues, i, j);
					}
					else if (j == 3)
					...
					else if (j == 6)
					{
						...
						// sum product algorithm
						applySumProduct(_prob, otherValues3, i, j);
						// max product algorithm
						applyMaxProduct(_prob_max, otherValues, i, j);
					}
				}
...
void applyMaxProduct(std::vector<std::vector<std::vector<double>>> &prob,
	...
	x1 = Util::instance()->returnBiggerValue(
		...
		prob[i][otherValues[0]][1] * prob[i][otherValues[1]][1] * prob[i][otherValues[2]][1],
		prob[i][otherValues[0]][1] * prob[i][otherValues[1]][0] * prob[i][otherValues[2]][0]),
		Util::instance()->returnBiggerValue(
		prob[i][otherValues[0]][0] * prob[i][otherValues[1]][1] * prob[i][otherValues[2]][0],
		prob[i][otherValues[0]][0] * prob[i][otherValues[1]][0] * prob[i][otherValues[2]][1]));
	...
	x0 = Util::instance()->returnBiggerValue(
		Util::instance()->returnBiggerValue(
		prob[i][otherValues[0]][1] * prob[i][otherValues[1]][0] * prob[i][otherValues[2]][1],
		prob[i][otherValues[0]][1] * prob[i][otherValues[1]][1] * prob[i][otherValues[2]][0]),
		Util::instance()->returnBiggerValue(
		prob[i][otherValues[0]][0] * prob[i][otherValues[1]][1] * prob[i][otherValues[2]][1],
		prob[i][otherValues[0]][0] * prob[i][otherValues[1]][0] * prob[i][otherValues[2]][0]));

	// normalize the x1 and x0
	x1 = x1 / (x0 + x1);
	x0 = x0 / (x0 + x1);

	prob[i][j][0] = Util::instance()->returnBiggerValue(prob[i][j][0], x0);
	prob[i][j][1] = Util::instance()->returnBiggerValue(prob[i][j][1], x1);
}
